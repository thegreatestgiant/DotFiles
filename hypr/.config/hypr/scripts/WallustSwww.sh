...
rofi_link="$HOME/.config/hypr/rofi/.current_wallpaper"
...
start_ts=$(date +%s)
wallust run -d ~/.config/hypr/wallust/ -s "$wallpaper_path" || true
wallust_targets=(
    "$HOME/.config/hypr/waybar/wallust/colors-waybar.css"
    "$HOME/.config/hypr/rofi/wallust/colors-rofi.rasi"
)
